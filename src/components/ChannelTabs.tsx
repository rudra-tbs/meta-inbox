...
export default function ChannelTabs({ activeChannel }: ChannelTabsProps) {
  return (
    <div className="flex gap-1 pb-2">
      ...
      <button
        className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
          activeChannel === 'WA'
            ? 'text-rose-600 border-b-2 border-rose-600 rounded-none'
            : 'text-slate-500'
        }`}
      >
        <svg
      ...
      {/* Instagram tab — Phase 2 */}
      <button
        disabled
        title="Phase 2 — Coming Soon"
        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-300 rounded-md cursor-not-allowed"
      >
        Instagram
      </button>
    </div>
